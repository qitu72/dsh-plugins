...
- 📐 **顶边拖拽调高**——鼠标悬停到输入卡片顶边会出现一条中线把手，按住上下拖随意改输入区高度（48px ~ 70% 视窗），高度自动记忆、换会话不丢，双击把手一键复位
- ⏎ **Ctrl+Enter = 换行**——对齐 WorkBuddy / CodeBuddy 的肌肉记忆，再也不会一句话没打完就飞出去；普通 Enter 照旧发送，Shift+Enter 换行保持不变
...
<details>
<summary><b>输入框高度怎么调？不小心拖乱了怎么办？</b></summary>

鼠标悬停到输入卡片顶边，出现中线把手后按住上下拖即可（范围 48px ~ 70% 视窗），高度自动记忆、换会话保留；**双击顶边**一键恢复默认高度。
</details>
